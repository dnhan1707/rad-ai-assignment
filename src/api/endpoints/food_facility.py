...
async def search_by_name(name: str, status: Optional[str] = None):
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Error with search_applicant_by_name, {str(e)}')
...
async def search_by_street_name(name: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Error with search_applicant_by_street_name, {str(e)}')
...
async def search_by_lat_lng(lat: float, lng: float, k: int, status: str = "APPROVED"):
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Error with search_applicant_by_lat_lng, {str(e)}')
